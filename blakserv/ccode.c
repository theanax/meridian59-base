			...
			int num_name_parms,parm_node name_parm_array[])
{
	...
	if (c == NULL)
	{
		bprintf("C_Broadcast can't find class %i, can't print out debug strs\n",
			kstat->interpreting_class);
		return NIL;
	}
		...
		case TAG_STRING :
			{
				...
				if (snod == NULL)
				{
					bprintf("C_Broadcast can't find string %i\n",each_val.v.data);
					return NIL;
				}
				lenBuffer = strlen(buf);
				memcpy(buf + lenBuffer,snod->data,snod->len_data);
			  ...
			  int num_name_parms,parm_node name_parm_array[])
{
    ...
    */
	...
	ret_val.v.data = GetTime() - 1690441200L;    // Offset to Nov 2017
